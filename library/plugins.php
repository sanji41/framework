...
function framework_get_socialimage() {
  global $post, $posts;
...
function framework_facebook_connect() {
	...
	echo '<meta property="og:image" content="'. framework_get_socialimage() .'"/>' . "\n";
	echo '<!-- end facebook open graph -->' . "\n";
}

// google +1 meta info
function framework_google_header() {
	if (is_singular()) {
		echo '<!-- google +1 tags -->' . "\n";
		global $post;
		echo '<meta itemprop="name" content="'.get_the_title().'">' . "\n";
		echo '<meta itemprop="description" content="' .strip_tags( get_the_excerpt() ).'">' . "\n";
		echo '<meta itemprop="image" content="'. framework_get_socialimage() .'">' . "\n";
		echo '<!-- end google +1 tags -->' . "\n";
	}
}
	
	// add this in the header 
	add_action('wp_head', 'framework_facebook_connect');
	add_action('wp_head', 'framework_google_header');
...
function framework_add_user_fields( $contactmethods ) {
	// Add Facebook
...
add_filter('user_contactmethods','framework_add_user_fields',10,1);
?>